...
const dentistaController = {
    ...
    getAll: async (req, res) => {
        try {
            const dentistas = await DentistaModel
                .find()
                .populate('user')
    ...
    delete: async (req, res) => {
        try {
            ...
            console.log(error)
            res.status(500).json({ error: "Ocorreu um erro ao apagar os dados do Dentista" })
        }
    },
    update: async (req, res) => {
        try {
            ...
            const dentista = await UsuarioModel
                .findByIdAndUpdate(updatedDentista.user._id, {
                    email,
                    senha
                }, { new: true })
            ...
            console.log(error)
            res.status(500).json({ error: "Ocorreu um erro ao atualizar os dados do Dentista" })
        }
    }
}
